...
public class PatternSystem : MonoBehaviour {
	public int sceneNumber; // the number (as per build settings) of the scene which this pattern manager will enable
	public int firstSceneNumber; // this is the default gamePlay first scene -- used as a trigger -- need to integrate this flag better with the project
	public bool isTesting = false; // when true, the game exhibits more moving_obstacles
	public int indexOfTestingItem = 0; // set this to an item you want to see a lot of while testing
	public int indexOfReplaceableItem = 3; // index number of the item you want to replace with testing items
	public int indexOfTestingItem2 = 1; // set this to an item you want to see a lot of while testing
	public int indexOfReplaceableItem2 = 2; // index number of the item you want to replace with testing items
	...
	//GameObject
	public List<GameObject> building_Obj = new List<GameObject>();
	public List<GameObject> item_Obj = new List<GameObject>();
	public List<GameObject> floor_Obj = new List<GameObject>();
	...
	public void Start(){
		...
		// set the active patternManager in the scene to the static instance for other scripts to access
		if (this.gameObject.activeInHierarchy) {
			instance = this.gameObject.GetComponent<PatternSystem>();
			StartCoroutine(CalAmountItem());
			}
		}

	void Update(){
		// set the static pattern manager to the active on in the scene (controlled by scene change in GameController)
		...
		while(i < item_Pref.Count){
			...
			while(j < amountItemSpawn[i]){
				...
				// ######################################################################################  EDITING
					...
					if (i == indexOfReplaceableItem && (item_Pref.Count >= indexOfTestingItem)){
						go = (GameObject)Instantiate(item_Pref[indexOfTestingItem], posStart, Quaternion.identity);
				}
					else if (i==indexOfReplaceableItem2 && (item_Pref.Count >= indexOfTestingItem2)){
						go = (GameObject)Instantiate(item_Pref[indexOfTestingItem], posStart, Quaternion.identity);
				}
				else{
					go = (GameObject)Instantiate(item_Pref[i], posStart, Quaternion.identity);
				}
				// ######################################################################################  EDITING
				// ## to return to normal, uncomment the following line and remove everything between "###...EDITING" comments
				}
				else{
					go = (GameObject)Instantiate(item_Pref[i], posStart, Quaternion.identity);
				}
