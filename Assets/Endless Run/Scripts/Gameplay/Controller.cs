...
public class Controller : MonoBehaviour {
	...
	public float deathHeight = -2f; // height below which the player dies
	...
	IEnumerator UpdateAction(){
		while(GameAttribute.gameAttribute.life > 0){//  && transform.position.y > deathHeight){
			if(GameAttribute.gameAttribute.pause == false && GameAttribute.gameAttribute.isPlaying &&
			   PatternSystem.instance.loadingComplete && GameController.instace.sceneReady){
	...
	private void KeyInput()
	{
		...
		if(activeInput && checkSideCollision == false)
		{
				...
				if(Input.GetKey(KeyCode.S))
			{
				...
				if (Input.GetKeyDown("space"))// if(Input.GetKey(KeyCode.Space))
			{
				//Debug.Log("Toggle weapon");
				// grab weapon and toggle it
				Weapon temp = GetComponentInChildren <Weapon>();
				//Debug.Log(temp);
				temp.toggle();
			}
		}else{
